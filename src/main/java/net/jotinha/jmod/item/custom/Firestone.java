package net.jotinha.jmod.item.custom;


import net.jotinha.jmod.util.JTags;
...
public class Firestone extends Item {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if(Screen.hasShiftDown()){
            tooltip.add(new TranslationTextComponent("tooltip.jmod.firestone_shift"));
        } else{
            tooltip.add(new TranslationTextComponent("tooltip.jmod.firestone"));
        }
    ...
    private void rightClickOnCertainBlockState(BlockState clickedBlock, ItemUseContext context,
        ...
        if (random.nextFloat() > 0.5f) {
            ...
            //bloco quebra, player ganha fire resistence

        } else {
            lightGroundOnFire(context);
            //se nenhum for verdadeiro, fogo no chão

        }
    }
    //CHECA SE O BLOCO É VALIDO
    private boolean blockIsValidForResistance(BlockState clickedBlock) {
        return clickedBlock.isIn(JTags.Blocks.FIRESTONE_CLICKABLE_BLOCKS);
    }
    ...
    private void gainFireResistanceAndDestroyBlock(PlayerEntity playerEntity, World world, BlockPos pos){
        ...
        //p_175655_2 = dropblock
        world.destroyBlock(pos, true);
    }
    public static void gainFireResistance(PlayerEntity playerEntity){
        playerEntity.addPotionEffect(new EffectInstance(Effects.FIRE_RESISTANCE, 120));
    }
